    ...
    <div style={{display:"flex",minHeight:"100vh",background:T.bg,fontFamily:"'DM Sans',sans-serif",color:T.text}}>
      ...
      {modal?.type==="edit_order" && (
        <EditOrderModal
          ...
          onSave={async (o) => {
            try {
              await apiFetch("/orders/" + modal.order.id, { method: "PUT", body: o });
              const client = clients.find(c => c.id === +o.client_id);
              const vehicle = vehicles.find(v => v.id === +o.vehicle_id);
              const mechanic = users.find(u => u.id === +o.mechanic_id);
              setOrders(p => p.map(x => x.id === modal.order.id ? {
                ...x,
                ...o,
                client_name: client?.name || x.client_name,
                client_phone: client?.phone || x.client_phone,
                make: vehicle?.make || x.make,
                model: vehicle?.model || x.model,
                plate: vehicle?.plate || x.plate,
                mechanic_name: mechanic?.name || x.mechanic_name,
                items: o.items || x.items,
              } : x));
              setModal(null);
            } catch (err) {
